		...
		for sub in subs:
			...
			if extension == ".srt":
				...
				if parse == None:
					...
					parse = re.search(r"(.+).(S\d{2}E\d{2})", name)
				# Tries to rebuild epcode as Sso as not to have to switch
				# around main logic to support more parse group 3
				if parse == None:
					print("Trying 3rd regex")
					parse = re.search(r"(.+) - (\d{2})x(\d{2})", name)
					fixedName = parse.group(
						1) + " - S" + parse.group(2) + "E" + parse.group(3)
				...
				if parse:
					...
					try:
						...
						c.execute("update bigtable set srt = (?) where epcode = (?) and showname = (?)", [
								  newSub, epcode, showName])
					except:
	...
	def pathBuilder(self, showFolder):
		...
		for root, dirs, files in walk:
			for video in files:
				...
				filepath = root + "/" + video
				# Following REGEX matches epcode as group 2 for both S01E01-E02
				# and S01E01 - S01E02
				try:
					parse = re.search(
						r"^.+\/(.+?) - (S\d{2}E\d{2}(-E\d{2}| - S\d{2}E\d{2}|)) - (.+)", filepath)
				...
				try:
					c.execute("INSERT INTO bigtable VALUES (?,?,?,?,?)", [
							  srt, epcode, length, filepath, showName])
				except:
...
def ripSub(filename):
	...
	try:
		out = subprocess.check_call(["ffmpeg", "-i", filename, "-map", "0:s:0", "tmp.srt", "-y"],
...
def extractSubs(conn, dir, show, season=None):
	...
	if c:
		...
		for path, srt in results:
			...
			if parse:
				# some things are still being stored as binary instead
				# plaintext in db
				try:
					title = parse.group(1)
					tmp = str(srt)
