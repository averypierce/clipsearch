...
# Set preamble to number of subtitle lines you want to play ahead of your
# search term
...
def VLC(vlc, path, timeIndex, platform):
    ...
    try:
        ...
        if platform == 'mixed':
            # chdir to suppress 'unable to translate working directory message'
        ...
        command = '"' + vlc + '" "' + videoPath + \
            '" --start-time=' + str(timeIndex)
    ...
    while conn:
        ...
        try:
            ...
            if matches:
                ...
                if(matches.length() > 1):
                    choice = int(
                        input("Type digit for desired episode and press enter: "))
